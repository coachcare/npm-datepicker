...
export function coerceDateProperty<D>(adapter: DateAdapter<D>, value: any): D | null {
  if (typeof value === 'string') {
    value = !value ? null : adapter.fromIso8601(value) || value;
  }
  if (!value || adapter.isDateInstance(value)) {
    return value;
  }
  throw Error(
    `Datepicker: Value must be either a date object recognized by the DateAdapter or ` +
